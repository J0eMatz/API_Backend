...
module.exports = {
  ...
  delete: async (req, res, next) => {
    ...
    try {
      ...
      next({
        status: 400,
        send: { msg: "No se pudo eliminar el usuario", err: error },
      });
    }
  },
  put: async (req, res, next) => {
    ...
    try {
      ...
      next({
        status: 201,
        send: { msg: "Usuario actualizado correctamente", data: {} },
      });
    } catch (error) {
      next({
        status: 400,
        send: { msg: "No se pudo actualizar el usuario", err: error },
      });
    }
  },
  login: async (req, res, next) => {
    try {
      ...
      next({
        status: 200,
        send: { msg: "Acceso autorizado", token: token },
      });
    } catch (error) {
